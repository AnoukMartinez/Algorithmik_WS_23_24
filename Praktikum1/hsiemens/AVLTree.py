...
class AVLTree:
    ...
    def find_most_likely_ngrams(self, word: str):
        ...
        ngrams_dict = {}
        searched_nodes = self._find_most_likely_ngrams(word, self.root, ngrams_dict)
        return ngrams_dict, searched_nodes

    #custom_def
    def _find_most_likely_ngrams(self, word: str, cur_node, ngrams_dict: dict):
        searched_nodes = 0

        if cur_node is not None:
            # If the current node's key starts with the given word, it's a possible continuation
            if cur_node.key.startswith(word):
                ngrams_dict[cur_node.key] = cur_node.values

            # If the given word is less than the current node's key, search in the left subtree
            if word < cur_node.key:
                searched_nodes += 1
                searched_nodes += self._find_most_likely_ngrams(word, cur_node.left_child, ngrams_dict)

            # If the given word is greater than the current node's key, search in the right subtree
            if word > cur_node.key:
                searched_nodes += 1
                searched_nodes += self._find_most_likely_ngrams(word, cur_node.right_child, ngrams_dict)
